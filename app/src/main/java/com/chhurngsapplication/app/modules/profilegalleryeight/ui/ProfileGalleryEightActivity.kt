package com.chhurngsapplication.app.modules.profilegalleryeight.ui

import android.view.View
import androidx.activity.viewModels
import com.chhurngsapplication.app.R
import com.chhurngsapplication.app.appcomponents.base.BaseActivity
import com.chhurngsapplication.app.databinding.ActivityProfileGalleryEightBinding
import com.chhurngsapplication.app.modules.nathanone.ui.NathanOneActivity
import com.chhurngsapplication.app.modules.profilegalleryeight.`data`.model.Staggered2RowModel
import com.chhurngsapplication.app.modules.profilegalleryeight.`data`.viewmodel.ProfileGalleryEightVM
import kotlin.Int
import kotlin.String
import kotlin.Unit

class ProfileGalleryEightActivity :
    BaseActivity<ActivityProfileGalleryEightBinding>(R.layout.activity_profile_gallery_eight) {
  private val viewModel: ProfileGalleryEightVM by viewModels<ProfileGalleryEightVM>()

  override fun onInitialized(): Unit {
    viewModel.navArguments = intent.extras?.getBundle("bundle")
    val staggeredAdapter = StaggeredAdapter(viewModel.staggeredList.value?:mutableListOf())
    binding.recyclerStaggered.adapter = staggeredAdapter
    staggeredAdapter.setOnItemClickListener(
    object : StaggeredAdapter.OnItemClickListener {
      override fun onItemClick(view:View, position:Int, item : Staggered2RowModel) {
        onClickRecyclerStaggered(view, position, item)
      }
    }
    )
    viewModel.staggeredList.observe(this) {
      staggeredAdapter.updateData(it)
    }
    binding.profileGalleryEightVM = viewModel
  }

  override fun setUpClicks(): Unit {
    binding.linearColumnrectanglethree.setOnClickListener {
      val destIntent = NathanOneActivity.getIntent(this, null)
      startActivity(destIntent)
    }
  }

  fun onClickRecyclerStaggered(
    view: View,
    position: Int,
    item: Staggered2RowModel
  ): Unit {
    when(view.id) {
    }
  }

  companion object {
    const val TAG: String = "PROFILE_GALLERY_EIGHT_ACTIVITY"

  }
}
